		...
		return {
			...
			init: function (params) {
				if(isWindowOpen() || jqcc('#cometchat_container_'+name).length > 0) {
					alert("<?php echo $audiochat_language['popup_already_open'];?>");
					return;
				}
				var id = params.to;
						...
						if(chatroommode == 1){
							...
							jqcc.ajax({
								...
								success : function(data){
									...
									var controlparameters = {"type":"plugins", "name":"core", "method":"loadCCPopup", "params":{"url": baseUrl+'plugins/audiochat/index.php?action=call&chatroommode=1&type=0&join=1&grp='+id+'&basedata='+baseData, "name":"audiochat", "properties":"status=0,toolbar=0,menubar=0,directories=0,resizable=1,location=0,status=0,scrollbars=0, width=<?php echo $width;?>,height=<?php echo $height;?>", "width":"<?php echo $width;?>", "height":"<?php echo $height;?>", "title":'<?php echo $audiochat_language[8];?>', "force":"0", "allowmaximize":"1", "allowresize":"1", "allowpopout":"1", "windowMode":windowMode}};
			...
			accept: function (params) {
				...
				if(supported){
					...
					if(mobileDevice == null) {
						loadCCPopup(baseUrl+'plugins/audiochat/index.php?action=call&caller='+caller+'&grp='+grp+'&basedata='+baseData+'&to='+id, 'audiochat',"status=0,toolbar=0,menubar=0,directories=0,resizable=1,location=0,status=0,scrollbars=0, width=<?php echo $camWidth;?>,height=<?php echo $camHeight;?>",<?php echo $camWidth;?>,<?php echo $camHeight;?>,'<?php echo $audiochat_language[8];?>',0,1,allowresize,1,windowMode);
					} else {
						loadCCPopup(baseUrl+'plugins/audiochat/index.php?action=call&caller='+caller+'&grp='+grp+'&basedata='+baseData+'&to='+id, 'audiochat',"status=0,toolbar=0,menubar=0,directories=0,resizable=1,location=0,status=0,scrollbars=0, width=<?php echo $camWidth;?>,height=<?php echo $camHeight;?>",<?php echo $camWidth;?>,<?php echo $camHeight;?>,'<?php echo $audiochat_language[8];?>',0,1,allowresize,1,1);
					}
				} else {
