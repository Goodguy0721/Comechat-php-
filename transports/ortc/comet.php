...
class Comet{
    ...
    function __construct($app_key, $priv_key){

		global $ortc_apikey;
		global $ortc_secretkey;
		$balancer = 'http://ortc-developers.realtime.co/server/2.1';
		$token = rand(0,9999999);

		if (empty($ortc_apikey) || empty($ortc_secretkey)) die("ERROR: Some of constructor's parameters are missing.");

		$strpos_res = strpos( $balancer, "2.1" );
		$this->is_version_21 = ($strpos_res > 0);

		$this->ortc_data['balancer'] = $balancer;
		$this->ortc_data['app_key'] = $ortc_apikey;
		$this->ortc_data['priv_key'] = $ortc_secretkey;
	...
	private function send_message_part($url, $channel, $msg, &$response = Array() ){
		$msg = json_encode(unserialize($msg));
		$message = array(
	...
	public function publish($args, &$response = Array() ){

		$url = $this->_get_server();
		$channel = $args['channel'];
		$msg = serialize($args['message']);
