...
function datify($ts) {
	...
	if($diff == 0) {
		...
		if($day_diff == 0) {
		if($diff < 60) return ['just now',$date];
		if($diff < 120) return ['1 minute ago',$date];
		if($diff < 3600) return [floor($diff / 60) . ' minutes ago',$date];
		if($diff < 7200) return ['1 hour ago',$date];
		if($diff < 86400) return [floor($diff / 3600) . ' hours ago',$date];
	}
	if($day_diff == 1) { return ['Yesterday at '.date('g:ia',$ts),$date]; }

	    if (date('Y') == date('Y',$ts)) {
	      return [date('F jS',$ts).' at '.date('g:ia',$ts),$date];
	    } else {
	      return [date('F jS, Y',$ts).' at '.date('g:ia',$ts),$date];
	    }
