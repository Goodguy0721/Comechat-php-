...
function heartbeat() {
	...
	if ((empty($_SESSION['cometchat']['cometchat_chatroomslist'])) || $force==1 || (!empty($_SESSION['cometchat']['cometchat_chatroomslist']) && ($time-$_SESSION['cometchat']['cometchat_chatroomslist'] > REFRESH_BUDDYLIST))) {
		...
		if(!is_array($cachedChatrooms = getCache('chatroom_list'))|| ($force==1)) {
			...
			if($showChatroomUsers == 1){
				$sqlPart = "(select COUNT(cometchat_chatrooms_users.userid) online from cometchat_chatrooms_users left join cometchat_status on cometchat_chatrooms_users.userid = cometchat_status.userid where cometchat_chatrooms_users.chatroomid = cometchat_chatrooms.id and isbanned<>'1')";
			} else {
				$sqlPart = '0';
			}
			$sql = ("select DISTINCT cometchat_chatrooms.id, cometchat_chatrooms.name, cometchat_chatrooms.type, cometchat_chatrooms.livingwith, cometchat_chatrooms.password, cometchat_chatrooms.lastactivity, cometchat_chatrooms.invitedusers, cometchat_chatrooms.createdby, ".$sqlPart." online from cometchat_chatrooms order by name asc");
			$query = mysqli_query($GLOBALS['dbh'],$sql);

			while ($chatroom = mysqli_fetch_assoc($query)) {
				$cachedChatrooms[$chromeReorderFix.$chatroom['id']] = array(
                    'id' => $chatroom['id'], 
                    'name' => urldecode($chatroom['name']), 
                    ...
                    'livingwith' => !empty($chatroom['livingwith']) ? explode(',', $chatroom['livingwith']) : [],
                );
			}
		...
		foreach($cachedChatrooms as $key=>$chatroom) {
			if((($chatroom['createdby'] == 0 || ($chatroom['createdby'] <> 0 && $time - $chatroom['lastactivity'] < $chatroomTimeout)) || $chatroom['createdby'] == $userid) && ($chatroom['type'] <> 3)) {
				...
				if($chatroom['type'] == 2 && !in_array($userid, $userList) && $chatroom['createdby'] != $userid){
					...
					$chatroomList[$chromeReorderFix.$chatroom['id']] = array(
                        'id' => $chatroom['id'], 
                        'name' => $chatroom['name'], 
                        'online' => $chatroom['online'], 
                        'type' => $chatroom['type'], 
                        'livingwith' => $chatroom['livingwith'],
                        'i' => $chatroom['password'], 
                        's' => $s, 
                        'createdby' => $chatroom['createdby'], 
                        'j' =>  $joined
	...
	if (count($joinedrooms) > 0 || (!empty($_REQUEST['callbackfn']) && $_REQUEST['callbackfn']=='mobileapp')) {
		...
		if($force == 1 && !empty($_SESSION['cometchat']['cometchat_chatroom_'.$_POST['currentroom']]) && !(USE_COMET == 1 && COMET_CHATROOMS == 1)){
			...
			if(!empty($_REQUEST['callbackfn']) && $_REQUEST['callbackfn']=='mobileapp' && empty($_REQUEST['v']) || !empty($joinedrooms)){
				$timestampCondition = "";
				if ($_POST['timestamp'] != 0) {
					$timestampCondition = " cometchat_chatroommessages.chatroomid in('".implode("','", $joinedrooms)."') and cometchat_chatroommessages.id > '".mysqli_real_escape_string($GLOBALS['dbh'],$_POST['timestamp'])."' and ";
					$limitClause = "";
				} else {
...
function getchatroomusers() {
	...
	if(!is_array($users = getCache('chatrooms_users'.$_POST['currentroom'])) || ($force == 1)) {
		...
		while ($chat = mysqli_fetch_assoc($query)) {
			...
			$users[$chromeReorderFix.$chat['userid']] = array('id' => (int)$chat['userid'], 'n' => $chat['username'], 'a' => $avatar, 'b' => $chat['isbanned'], 'chatroomid' => $_POST['currentroom']);
		}
...
function deletechatroom(){
	...
	if (!empty($_POST['id'])) {
		...
		$controlparameters = array('type' => 'modules', 'name' => 'chatroom', 'method' => 'deletedchatroom', 'params' => array('id' => $_POST['id']));
		$controlparameters = json_encode($controlparameters);
		sendChatroomMessage($_POST['id'],'CC^CONTROL_'.$controlparameters,0);
...
if (!empty($_GET['action']) && in_array($_GET['action'],$allowedActions)) {
	if($_GET['action']=='getChatroomDetails' && !empty($_REQUEST['id'])){
		call_user_func($_GET['action'],$_REQUEST['id']);
	}else{
