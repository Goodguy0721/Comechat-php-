    ...
    $.crdocked = (function() {
        return {
                ...
                loadRoom: function(clicked,id,minimized,unreadmsgcount) {
                    ...
                    if(chatroomsOpened[roomno]!=null){
                        if($("#cometchat_group_"+id+"_popup").hasClass('cometchat_tabhidden')){
                    ...
                    if(widthavailable < (chatboxWidth+chatboxDistance)){
                        var closingchatbox = $('#cometchat_chatboxes').find('span.cometchat_tabopen_bottom:first-child').attr('userid');

                        if(typeof closingchatbox != "undefined"){
                            jqcc.docked.closeChatbox(closingchatbox);
                        } else {
                            closingchatbox = $('#cometchat_chatboxes').find('span.cometchat_tabopen_bottom:first-child').attr('groupid');
                    ...
                    jqcc('#cometchat_sendmessagebtn').click(function(e){
                        ...
                        cometchat_grouptextarea.addClass('placeholder');
                ...
                inviteGroupUsers: function(groupid) {
                    var baseurl = jqcc.cometchat.getBaseUrl();
                    var basedata = jqcc.cometchat.getBaseData();
                ...
                loadChatroomList : function(item) {
                    ...
                    if(document.getElementById('cometchat_groupslist_content')){
                        ...
                        if(mobileDevice){
                            ...
                            userstabpopup.find('#cometchat_userscontent #cometchat_groupslist_content > div').css({'height': grouplistheight});
                            userstabpopup.find('#cometchat_userscontent #cometchat_groupslist_content > div').slimScroll({height: grouplistheight});
                        }
                    }
                },
                displayChatroomMessage: function(item,fetchedUsers) {
                    ...
                    $.each(item, function(i,incoming) {
                        ...
                        if (message != '') {
                                ...
                                var prepend = '';
                                    prepend = '<div class=\"cometchat_prependCrMessages\" onclick\="jqcc.crdocked.prependCrMessagesInit('+incoming.chatroomid+')\" id = \"cometchat_prependCrMessages_'+incoming.chatroomid+'\"><?php echo $chatrooms_language[74];?></div>';
                                ...
                                if($("#cometchat_groupmessage_"+incoming.id).length > 0) {
                                    ...
                                    if(incoming.fromid != settings.myid) {
                                        ...
                                        temp += prepend+'<div class="cometchat_time cometchat_time_'+msg_date_class+' '+date_class+'" msg_format="'+msg_date_format+'">'+msg_date+'</div><div class="cometchat_chatboxmessage" id="cometchat_groupmessage_'+incoming.id+'">'+fromavatar+'<div class="'+add_bg+' '+'cometchat_ts_margin cometchat_floatL" style="'+add_style+'"><span id="cc_groupmessage_'+incoming.id+'" class="cometchat_msg">'+message+'</span></div>'+sentdata_box+' '+add_arrow_class+'</div>';
                                        beepNewMessages++;
                                    } else {
                                        var sentdata = $[calleeAPI].getTimeDisplay(ts,incoming.id);
                                        var sentdata_box = "<span class=\"cometchat_ts\">"+sentdata+"</span>";
                                ...
                                grouppopup.find('.cometchat_prependCrMessages').hide();
                                ...
                                $.each($("#cometchat_grouptabcontenttext_"+incoming.chatroomid+" .cometchat_prependCrMessages"),function (i,divele){
                                    $("#cometchat_grouptabcontenttext_"+incoming.chatroomid+" .cometchat_prependCrMessages:first").show();
                                });
                                if((message).indexOf('<img')!=-1 && (message).indexOf('src')!=-1){
                    ...
                    prependCrMessages:function(id,data){
                            ...
                            if (message != '') {
                                ...
                                if (incoming.fromid != settings.myid) {
                                   ...
                                   if((message.indexOf('<img')==-1 && message.indexOf('src')==-1) || (smileycount > 0 && smileymsg != '')){
                                    ...
                                    temp += prepend+'<div class="cometchat_time cometchat_time_'+msg_date_class+' '+date_class+'" msg_format="'+msg_date_format+'">'+msg_date+'</div><div class="cometchat_chatboxmessage" id="cometchat_groupmessage_'+incoming.id+'">'+fromavatar+'<div class="'+add_bg+' '+'cometchat_ts_margin cometchat_floatL"><span id="cc_groupmessage_'+incoming.id+'" class="cometchat_msg">'+message+'</span></div>'+sentdata_box+' '+add_arrow_class+deleteMessage+'</div>';
                                } else {
                                    var sentdata = $[calleeAPI].getTimeDisplay(ts,incoming.id);
                                    var sentdata_box = "<span class=\"cometchat_ts\">"+sentdata+"</span>";
                                    if((message.indexOf('<img')==-1 && message.indexOf('src')==-1) || (smileycount > 0 && smileymsg != '')){
                        ...
                        if((count - parseInt(settings.prependLimit) < 0)){
                            ...
                            jqcc('.cometchat_prependCrMessages').attr('onclick','jqcc.docked.prependCrMessagesInit('+id+')');
                        }
                        $.each($('#cometchat_group_'+id+'_popup .cometchat_chatboxmessage'),function (i,divele){
                            if($(this).find(".cometchat_ts_margin").next().hasClass("cometchat_ts")){
                                var msg_containerHeight = $(this).find(".cometchat_ts_margin").outerHeight();
                        ...
                        $.each($("#cometchat_grouptabcontenttext_"+id+" .cometchat_prependCrMessages"),function (i,divele){
                            $("#cometchat_grouptabcontenttext_"+id+" .cometchat_prependCrMessages:first").show();
                        });
