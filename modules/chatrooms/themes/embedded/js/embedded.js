    ...
    $.crembedded = (function() {
        return {
                ...
                addChatroomMessage: function(fromid,incomingmessage,incomingid,selfadded,sent,fromname,calledfromsend,chatroomid){
                    ...
                    $("#currentroom").find(".cometchat_prependCrMessages").hide();
                    ...
                    if(message != '' && chatroomid == jqcc.cometchat.getChatroomVars('currentroom')) {
                        ...
                        if($("#cometchat_groupmessage_"+incomingid).length > 0) {
                            ...
                            if(fromid != settings.myid) {
                                ...
                                temp += (prepend+'<div class="cometchat_time cometchat_time_'+msg_date_class+' '+date_class+'" msg_format="'+msg_date_format+'">'+msg_date+'</div><div style="clear:both;"></div><div class="cometchat_messagebox" id="cometchat_messagebox_'+incomingid+'">'+fromavatar+'<div class="'+add_bg+'" id="cometchat_groupmessage_'+incomingid+'">');
                                temp += ('<span class="cometchat_chatboxmessagecontent" style="'+imagemsg_rightcss+'">'+message+'</span></div>'+sentdata+'</div><div style="clear:both;"></div>');
                            } else {
                                temp += (prepend+'<div class="cometchat_time cometchat_time_'+msg_date_class+' '+date_class+'" msg_format="'+msg_date_format+'">'+msg_date+'</div><div style="clear:both;"></div><div class="cometchat_messagebox cometchat_messagebox_self" id="cometchat_messagebox_'+incomingid+'"><div class="'+add_bg+add_bg_self+'" id="cometchat_groupmessage_'+incomingid+'"><span class="cometchat_chatboxmessagecontent" style="'+imagemsg_rightcss+'">'+message+'</span></div>'+sentdata+'</div><div style="clear:both;></div>"');
                            }
                    ...
                    $.each($("#currentroom").find(".cometchat_prependCrMessages"),function (i,divele){
                        $("#currentroom .cometchat_prependCrMessages:first").show();
                    });
                ...
                createChatroomSubmitStruct: function() {
                    ...
                    var livingwith = document.getElementById('livingwith').value;
                ...
                loadChatroomList : function(item) {
                    ...
                    $.each(item, function(i,room) {
                        ...
                        if ($.inArray(lw, room.livingwith) == -1) {
                            return;
                        }
                        ...
                        if (message != '' && incoming.chatroomid == jqcc.cometchat.getChatroomVars('currentroom')) {
                                ...
                                if($("#cometchat_groupmessage_"+incoming.id).length > 0) {
                                    ...
                                    if(incoming.fromid != settings.myid) {
                                        ...
                                        temp += (prepend+'<div class="cometchat_time cometchat_time_'+msg_date_class+' '+date_class+'" msg_format="'+msg_date_format+'">'+msg_date+'</div><div style="clear:both;"></div><div class="cometchat_messagebox" id="cometchat_messagebox_'+incoming.id+'">'+fromavatar+'<div class="'+add_bg+'" id="cometchat_groupmessage_'+incoming.id+'">');

                                        temp += ('<span class="cometchat_chatboxmessagecontent" style="'+imagemsg_rightcss+'">'+message+'</span></div>'+$[calleeAPI].getTimeDisplay(ts,incoming.from,0)+'</div><div style="clear:both;"></div>');
                                        beepNewMessages++;
                                        deletemessagecss = 'style="'+deleteiconcss+'"';
                                        if($.cookie(settings.cookiePrefix+"sound") && $.cookie(settings.cookiePrefix+"sound") == 'true'){
                                            jqcc[calleeAPI].playsound();
                                        }
                                    } else {
                            ...
                            if ( $('#currentroom_convotext').html() == '') {
                                ...
                                $('#currentroom_convotext').append(prepend);
                            }
...
/*
                    ...
                    prependCrMessages:function(id,data){
                        ...
                        $.each(data, function(i, incoming){
                            ...
                            if (message != '') {
                                ...
                                if (incoming.fromid != settings.myid) {
                                    ...
                                    temp += ('<span class="cometchat_chatboxmessagecontent" style="'+imagemsg_rightcss+'">'+message+'</span></div>'+$[calleeAPI].getTimeDisplay(ts,incoming.from,0)+''+deleteMessage+'</div><div style="clear:both;"></div>');
                                } else {
                                    if (jqcc.cometchat.getChatroomVars('owner') || jqcc.cometchat.getChatroomVars('isModerator') || (incoming.fromid == settings.myid && jqcc.cometchat.getChatroomVars('allowDelete') == 1)) {
                        ...
                        $('.cometchat_prependCrMessages').text('<?php echo $chatrooms_language[74];?>');
                        if((count - parseInt(settings.prependLimit) < 0)){
                            $('.cometchat_prependCrMessages').text('<?php echo $chatrooms_language[75];?>');
                            jqcc('.cometchat_prependCrMessages').attr('onclick','');
                            jqcc('.cometchat_prependCrMessages').css('cursor','default');
                        }else{
                            jqcc('.cometchat_prependCrMessages').attr('onclick','jqcc.embedded.prependCrMessagesInit('+id+')');
                        }
                        $.each($("#currentroom").find(".cometchat_prependCrMessages"),function (i,divele){
                            $("#currentroom .cometchat_prependCrMessages:first").show();
                        });
                    ...
                    activeChatrooms: function(item){
                        ...
                        $.each(chatroomitem, function(i,room) {
                            ...
                            temp += '<div id="cometchat_chatroomlist_'+room.id+'" class="lobby_room'+selected+'" onmouseover="jqcc(this).addClass(\'cometchat_userlist_hover\');" onmouseout="jqcc(this).removeClass(\'cometchat_userlist_hover\');" onclick="javascript:jqcc.cometchat.chatroom(\''+room.id+'\',\''+urlencode(shortname)+'\',\''+room.type+'\',\''+room.i+'\',\''+room.s+'\',\'1\',\'1\');" style="display:block !important;"><div class="arrowright"><img src="themes/embedded/images/rightarrow.svg"></div><span class="lobby_room_3">'+roomtype+'</span><span class="lobby_room_4" title="<?php echo $chatrooms_language[58];?>" onclick="javascript:jqcc.cometchat.deleteChatroom(event,\''+room.id+'\');">'+deleteroom+'</span><span class="lobby_room_5" style="display:none;">'+roomowner+'</span><span class="lobby_room_6" title="<?php echo $chatrooms_language[80];?>" onclick="javascript:jqcc.'+[calleeAPI]+'.renameChatroom(event,\''+room.id+'\');">'+renameChatroom+'</span><div><span class="cometchat_chatroomimage"><img src="themes/<?php echo $theme;?>/images/group.svg"></span><span class="lobby_room_1" title="'+longname+'"><span class="currentroomname">'+longname+'</span><br/><span class="lobby_room_2" '+userCountCss+' title="'+room.online+' <?php echo $chatrooms_language[34];?>"><?php echo  $chatrooms_language["participants"]; ?>:&nbsp;'+room.online+'</span></span></div></span></div>';
...
jqcc('.cometchat_chatroomusers').live('click',function(){
    ...
    if(!jqcc('#cometchat_container_chatroomusers').hasClass('visible')){
        ...
        if (cometchat_container.hasClass('visible')){
            ...
            animatewidth = $(window).width()-500;
            var reducesize = 500;
            if(!$('.cometchat_windows').hasClass('visible') && !$('.cometchat_container').hasClass('visible')){
                if($("#cometchat_righttab").width()-500 <= 400 && $('#cometchat_righttab').width()!=$(window).width()){
                    if($(window).width() < 850){
                        cometchat_container.width(400);
                        reducesize = 400;
                        animatewidth = $(window).width() - cometchat_container.width();
                    }
                    $('#cometchat_leftbar').css('position','absolute');
                    $("#cometchat_righttab").animate({'width':$('#cometchat_righttab').width() - reducesize+'px','margin-left':'0px'},"fast");
                    $("#cometchat_righttab").find(".cometchat_textarea").animate({'width':$("#cometchat_righttab").width() - (reducesize + 140) + 'px'},"fast");
                    setTimeout(function(){
                        if(rtl == 1){
                            $('#cometchat_leftbar').animate({'left':'100%'},"fast");
                        }else{
                            $('#cometchat_leftbar').animate({'left':'-300px'},"fast");
                        }
                        $('#cometchat_leftbar').removeAttr('style');
                        $('#cometchat_leftbar').hide();
                    },100);
            ...
            if(rtl == 1){
                cometchat_container.animate({"right":animatewidth}, "fast").addClass('visible');
            }else{
                cometchat_container.animate({"left":animatewidth}, "fast").addClass('visible');
            }
            if($('#cometchat_righttab').css('top') == "0px" || $('#cometchat_header').length != 1){
            ...
            cometchat_container.find('.cometchat_closebox').click(function(){
                ...
                if($("#cometchat_righttab").width()+cometchat_container.width() >= ($(window).width()-2) && $('#cometchat_leftbar').css('left') != 'auto'){
                    ...
                    if(rtl == 1){
                        $('#cometchat_leftbar').animate({'right':'0px'},"fast");
                    }else{
                    ...
                    $("#cometchat_righttab").animate({'width':$("#cometchat_righttab").width()+windowwidth+'px'},"fast");
