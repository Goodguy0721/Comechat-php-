			...
			if (!is_array($buddyList = getCache($onlineCacheKey)) || ($_REQUEST['f'] == 1) || (defined('TAPATALK'))) {
				...
				while ($chat = mysqli_fetch_assoc($query)) {
					...
					if (!empty($chat['username']) && ($hideOffline == 0 || ($hideOffline == 1 && $chat['status'] != 'offline')) || in_array($chat['userid'],explode(",",$_REQUEST['activeChatboxIds']))) {
						$buddyList[$chromeReorderFix.$chat['userid']] = array('id' => $chat['userid'], 'n' => $chat['username'], 'l' => $link,  'a' => $avatar, 'd' => $chat['isdevice'], 's' => $chat['status'], 'm' => $chat['message'], 'g' => $chat['grp'], 'ls' => $chat['lastseen'], 'lstn' => $chat['lastseensetting'], 'ch' => $chat['ch'], 'id_country' => $chat['id_country'], 'id_region' => $chat['id_region'], 'id_city' => $chat['id_city'], 'livingwith' => explode(',', str_replace(['[',']'], '', $chat['living_with'])));
					}
				}
