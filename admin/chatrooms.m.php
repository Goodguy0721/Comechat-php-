...
$livingwith_options = [
    ...
    3 => 'HSV-2 (herpes type 2, usually genital)',
    4 => 'Herpes (not sure which type)',
...
function index() {
    ...
    while ($chatroom = mysqli_fetch_assoc($query)) {
        ...
        if (!empty($chatroom['livingwith'])) {
            $chatroom['livingwith'] = explode(',', $chatroom['livingwith']);
            $livingwith = [];
            foreach ($chatroom['livingwith'] as $value) {
                $livingwith[] = $livingwith_options[$value];
            }
            $livingwith = implode(';', $livingwith);
        } else {
            $livingwith = 'None';
        }

        $chatroomlist .= '<li class="ui-state-default" style="'.$css.'"><span style="font-size:11px;float:left;margin-top:3px;margin-left:5px;max-width: 400px;text-overflow: ellipsis;overflow: hidden;">'.$chatroom['name'].' (ID: '. $chatroom['id'].')'.$type.$typeuser.'('.$livingwith.')(<span class="chat_time" timestamp="'.$time.'"></span>)</span><p style="float:right"></p><span style="font-size:11px;float:right;margin-top:0px;margin-right:5px;">'.$extra.'<a href="?module=chatrooms&amp;action=deletechatroom&amp;data='.$chatroom['id'].'&amp;ts={$ts}"><img src="images/remove.png" title="Delete Chatroom"></a></span><div style="clear:both"></div></li>';
    }
    $errormessage = '';
    if(!$chatroomlist){
...
function newchatroom() {
    ...
    foreach ($livingwith_options as $lw_key => $lw_value) {
        $lw_options .= '<option value="' . $lw_key . '">' . $lw_value . '</option>';
    }
    ...
    <div id="rightcontent" style="float:left;width:720px;border-left:1px dotted #ccc;padding-left:20px;">
        ...
        <div>
            <div id="centernav">
                ...
                <div style="clear:both;padding:10px;"></div>
                <div class="title">Living with:</div><div class="element"><select id="chatroomlivingwith" multiple class="inputbox" name="livingwith[]">{$lw_options}</select></div>
            </div>
            <div id="rightnav">
                <h1>Warning</h1>
...
function newchatroomprocess() {
    ...
    $livingwith = mysqli_real_escape_string($GLOBALS['dbh'],implode(',', (array)$_POST['livingwith']));
    ...
    if(!empty($chatroom) && (strlen(trim($password)) !== 0 || empty($passwordhash))) {
        $sql = ("insert into cometchat_chatrooms (name,createdby,lastactivity,password,type,livingwith) values ('".mysqli_real_escape_string($GLOBALS['dbh'],sanitize_core($chatroom))."', '0','".mysqli_real_escape_string($GLOBALS['dbh'],getTimeStamp())."','".mysqli_real_escape_string($GLOBALS['dbh'],$passwordhash)."','".mysqli_real_escape_string($GLOBALS['dbh'],$type)."','".mysqli_real_escape_string($GLOBALS['dbh'],$livingwith)."')");
        $query = mysqli_query($GLOBALS['dbh'],$sql);
        header( "Location: ?module=chatrooms&ts={$ts}" );
    }
