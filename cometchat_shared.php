...
function setColorValue1($key,$val){
	global $color;
	$colordetails = array('primary_color' => array('color1'=>'56a8e3','color2'=>'4DC5CE','color3'=>'FFC107','color4'=>'FB4556','color5'=>'DBA0C3','color6'=>'3B5998','color7'=>'065E52','color8'=>'FF8A2E','color9'=>'E99090','color10'=>'23025E','color11'=>'24D4F6','color12'=>'289D57','color13'=>'D9B197','color14'=>'FF67AB','color15'=>'8E24AA'),'secondary_color' => array('color1'=>'3777A7','color2'=>'068690','color3'=>'FFA000','color4'=>'BB091A','color5'=>'D87CB3','color6'=>'213A6D','color7'=>'244C4E','color8'=>'CE610C','color9'=>'B55353','color10'=>'3D1F84','color11'=>'059EBB','color12'=>'09632D','color13'=>'C38B66','color14'=>'D6387E','color15'=>'7B1FA2') , 'hover_color' => array('color1'=>'ECF5FB','color2'=>'D3EDEF','color3'=>'FFF8E2','color4'=>'F5C3C8','color5'=>'ECD9E5','color6'=>'DFEAFF','color7'=>'AFCCAF','color8'=>'FDD9BD','color9'=>'FDE8E8','color10'=>'E5D7FF','color11'=>'DBF9FF','color12'=>'DDF9E8','color13'=>'FFF1E8','color14'=>'F3DDE7','color15'=>'EFE8FD'));

	if(!empty($key) && !empty($color)){
		return '#'.$colordetails[$key][$color];
	}else{
		return $val;
	}
...
function sendMessage($to,$message,$dir = 0,$type = '') {
	...
	if($dir === 0 && (empty($type) || ($type != 'filetransfer' && $type != 'handwrite' && $type != 'voicenote'))) {
	...
	if (!empty($to) && isset($message) && $message!='' && $userid > 0) {
		...
		if($dir === 0){
			if($stickersflag == 0 && $voicenoteflag == 0){
				$message = str_ireplace('CC^CONTROL_','',$message);
			}
		}
...
function sendChatroomMessage($to = 0,$message = '',$notsilent = 1) {
	...
	$flag = 0;
	$voicenoteflag = 0;
	if(($to == 0 && empty($_POST['currentroom'])) || ($message == '' && $notsilent == 0) || (isset($_POST['message']) && $_POST['message'] == '') || (empty($userid) && !defined('CCADMIN')) || in_array($userid, $bannedUserIDs)){
		return;
	}
