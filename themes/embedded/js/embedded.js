...
(function($){
    $.ccembedded = (function(){
        ...
        var livingwithvalue = 1;
        var locationvalue = '';
        var locationcountryvalue = 0;
        var locationregionvalue = 0;
        var locationcityvalue = 0;
        ...
        return {
            ...
            initialize: function(){
                ...
                for(i=0;i<settings.plugins.length;i++){
                    ...
                    if( name == 'ccblock' ){
                        var blockuserwindow = '<div id="cometchat_module_block" class="cometchat_module"><div class="cometchat_lightdisplay leftpadding"><div style="margin-top:2px;">'+language['blocked_users']+'</div><div class="module_arrowright"><img src="themes/embedded/images/right.svg" ></div><span id="cometchat_blockeduserscount"></span></div></div>';
                    }
                }
                ...
                if($(document).width() <= 414 ){
                    ...
                    headericon = newcompose+moreoption+notificationicon+livingwith /* +location */;
                }
                if(settings.ccauth.enabled=="1" || jqcc.cometchat.getCcvariable().callbackfn=='desktop'){
                ...
                var selfDetails = '<div id = "cometchat_header"><div id="cometchat_self_container">' + location + '<div id="cometchat_self_right">'+headericon+auth_logout+'</div><div id="cometchat_self_left"></div></div></div>';
                var recentgroup = '';
                var tabscontainer = '';
                var searchbar = '';
                var usertab = '';
                ...
                if(settings.showOnlineTab==1 && settings.enableType!=1){
                    ...
                    usertab = '<div id = "cometchat_chats"><span id="cometchat_chatstab" class="cometchat_tab tab_click" style="font-family:Lucida Grande/Tahoma;'+tabWidth+'right: 0;" ><span class="cometchat_tabstext">'+language["contacts"]+'</span></span></div>';
                    grouptab = '<div id = "cometchat_groups"><span id="cometchat_groupstab" class="cometchat_tab " style="font-family:Lucida Grande/Tahoma;'+tabWidth+'right: 0;" ><span class="cometchat_tabstext">'+language["groups"]+'</span></span></div>';
                ...
                if(chatroomsonly == 1){
                    ...
                    tabscontainer = usertab+grouptab;
                }
                ...
                usertabpop = '<div id="cometchat_leftbar">'+tabscontainer+searchbar+'<div id="cometchat_userscontent"><div id="cc_gotoPrevNoti"></div><div id="cc_gotoNextNoti"></div>'+chats+group+'</div></div>'+chatbox;
                var createChatroom = '';
                ...
                $( '#cometchat_header' ).on( 'click', '.float_list', function () {
                    ...
                    switch(id[2]){
                        ...
                        case language[113].replace(/\s+/, ""):
                            ...
                            $('.cometchat_float_list').hide();
                            var content = '<iframe style="border-top:1px solid #D1D1D1;height:100%;" frameborder="0" width="300" src="'+baseUrl+'modules/broadcastmessage/index.php?cc_theme=embedded&id=1"></iframe>';
                ...
                $(document).on('change', '[name=livingwith]', function(e) {
                    e.preventDefault();
                    livingwithvalue = $(this).val();
                    jqcc.cometchat.chatHeartbeat(1);
                    jqcc.cometchat.chatroomHeartbeat(1);
                });

                $(document).on('keyup', '[name=location]', function(e) {
                    e.preventDefault();
                    locationresults = [];
                    ...
                    locationtimeout = setTimeout(function() {
                        $.ajax({
                            url: '/countries/ajax_get_locations',
                            dataType: 'json',
                            type: 'POST',
                            data: {name: locationvalue},
                            cache: false,
                            success: function(resp) {
                                locationresults = resp.items.cities;
                                var html = '';
                                if (locationresults && locationresults.length > 0) {
                                    html += '<ul>';
                                    for (var i in locationresults) {
                                        html += '<li data-country="' + locationresults[i].country_code + '" data-region="' + locationresults[i].id_region + '" data-city="' + locationresults[i].id + '">' + locationresults[i].name + '</li>';
                                    }
                                    html += '</ul>';
                                }
                                $('#location-dropdown').html(html);
                            }
                        });
                    }, 500);
                });

                $(document).on('click', '#location-autocomplete #location-dropdown li', function(e) {
                    e.preventDefault();
                    locationvalue = $(this).html();
                    $('#location').find('[name=location]').val(locationvalue);
                    locationcountryvalue = $(this).data('country');
                    locationregionvalue = $(this).data('region');
                    locationcityvalue = $(this).data('city');
                    locationresults = [];
            ...
            buddyList: function(item){
                ...
                $.each(item, function(i, buddy){
                    ...
                    var lw = $('#livingwith').find('[name=livingwith]').val().toString();
                    if ($.inArray(lw, buddy.livingwith) == -1) {
                        return;
                    }

                    if (locationcountryvalue && buddy.id_country != locationcountryvalue) {
                        return;
                    }

                    if (locationregionvalue && buddy.id_region != locationregionvalue) {
                        return;
                    }

                    if (locationcityvalue && buddy.id_city != locationcityvalue) {
                        return;
                    }
            ...
            if(settings.enableType!=1 && embeddedchatroomid==0){
                ...
                if(typeof(cometchat_user_popup)=='undefined' || cometchat_user_popup.length<1){
                    ...
                    headerplugin = headerplugin+avchathtml+audiochat+'<div id="vline"><img src="themes/<?php echo $theme;?>/images/vline.svg"/ style="margin-top:12px;"></div> <div  class="cometchat_user_closebox"><img src="themes/<?php echo $theme;?>/images/remove.svg"/></div></div>';
                    var startlink = '';
                    var endlink = '';
                    if(link!=''){
            ...
            chatTab: function(){
                ...
                cometchat_user_search.keyup(function(){
                    var searchString = $(this).val();
                    if($('#cometchat_leftbar').find('#cometchat_chatstab').hasClass('cometchat_tabclick')){
                    ...
                    if($('#cometchat_leftbar').find('#cometchat_groupstab').hasClass('cometchat_tabclick')){
                    ...
                    if(params != null){
                        $.each(params, function(index, item){
                            if(item['g']) {
                                ...
                                if(item.hasOwnProperty('s') && typeof (jqcc[settings.theme].createChatboxData)!=='undefined'){
                                    if(item['s'] > 0){
            ...
            windowResize: function(silent){
                if(settings.enableType == 1 && chatroomsonly != 1){
                    ...
                    $('#livingwith').find('[name=livingwith]').val(livingwithvalue);
                }
            },
            ...
            prependMessages:function(id,data){
                    ...
                    if(type=="messages"){
                        $.each(item, function(i, incoming){
                           ...
                           if(incoming.sent!=null){
                            ...
                            if(jqcc.cometchat.getThemeVariable('prependLimit') != '0' && jabber != 1){
                                var prepend = '<div class=\"cometchat_prependMessages\" onclick\="jqcc.embedded.prependMessagesInit('+incoming.from+')\" id = \"cometchat_prependMessages_'+incoming.from+'\">'+language[83]+'</div>';
                            }
                ...
                if((count - parseInt(jqcc.cometchat.getThemeVariable('prependLimit')) < 0)){
                    ...
                    jqcc('#cometchat_prependMessages_'+id).attr('onclick','jqcc.embedded.prependMessagesInit('+id+')');
                }
