...
(function($){
    $.ccjabber = (function(){
        ...
        return {
            ...
            login: function(caller){
                ccjabber.hash = '';
                var baseUrl = $.cometchat.getBaseUrl();
                var baseData = $.cometchat.getBaseData();
                var baseDomain = document.domain;
                if($.cometchat.getCcvariable().callbackfn=='desktop'){
                    loadCCPopup(baseUrl+'extensions/jabber/index.php?basedata='+baseData+'&basedomain='+baseDomain, 'jabber','status=0,toolbar=0,menubar=0,directories=0,resizable=0,location=0,status=0,scrollbars=0, width=210,height=100',210,100,'<?php echo $connectPhrase;?>',null,null,null,null,1);
                }else{
                    var callerUrl = '';
                    if(typeof(caller) != "undefined" && caller != ''){
                        callerUrl = '&caller='+caller;
                    }
                    var controlparameters = {"type":"extensions", "name":"core", "method":"loadCCPopup", "params":{"url": baseUrl+"extensions/jabber/index.php?basedata="+baseData+"&basedomain="+baseDomain+callerUrl, "name":"jabber", "properties":"status=0,toolbar=0,menubar=0,directories=0,resizable=0,location=0,status=0,scrollbars=0, width=210,height=100", "width":"210", "height":"100", "title":"<?php echo $connectPhrase;?>", "force":null, "allowmaximize":null, "allowresize":null, "allowpopout":null, "windowMode":null}};
            ...
            process: function(caller){
                ...
                if(caller == '' || typeof(caller) == 'undefined'){
            ...
            sendMessage: function(id, message){
                ...
                $[ccjabber.theme].addMessages([{"from": id, "message": message, "self": 1, "old": 0, "id": currenttime, "selfadded": 1, "sent": null, "jabber":1}]);
                        ...
                        if(data){
                            if(data[0]&&data[0].error=='1'){
                                ...
                                $.each(data, function(id, message){
                                    ...
                                    $[ccjabber.theme].addMessages([{"from": message.from, "message": message.msg, "self": 0, "old": 0, "id": message.time, "selfadded": 0, "sent": null, "jabber":1}]);
                                    ccjabber.heartbeatTime = ccjabber.minHeartbeat;
                                });
 ...
 if(file_exists(dirname(__FILE__).DIRECTORY_SEPARATOR."themes".DIRECTORY_SEPARATOR.$theme.DIRECTORY_SEPARATOR."js".DIRECTORY_SEPARATOR.$theme.'.js')){
    ...
    include_once(dirname(__FILE__).DIRECTORY_SEPARATOR."themes".DIRECTORY_SEPARATOR.'docked'.DIRECTORY_SEPARATOR."js".DIRECTORY_SEPARATOR.'docked.js');
}
 ?>