...
jqcc.extend($.mobilewebapp, {
    ...
    loadChatroomList: function(item){
        ...
        $.each(item, function(i, room){
            var longname = room.name;
            var shortname = room.name;
            ...
            if(message != ''){
                if($("#cometchat_groupmessage_"+incoming.id).length>0){
                    ...
                    if(incoming.fromid!=jqcc.cometchat.getChatroomVars('myid')){
                        temp += ('<div><div class="cometchat_chatboxmessage you" id="cometchat_groupmessage_'+incoming.id+'"><span class="cometchat_chatboxmessagefrom"><strong>');
                        ...
                        temp += ('</strong></span><div class="userSeperator"></div><span class="cometchat_chatboxmessagecontent">'+message+'</span>'+jqcc.mobilewebapp.getTimeDisplay(ts)+'</div><div style="clear:both;"></div></div>');
                        ...
                        temp += ('<div><div class="cometchat_chatboxmessage me" id="cometchat_groupmessage_'+incoming.id+'"><span class="cometchat_chatboxmessagefrom"><strong>'+fromname+':</strong></span><div class="userSeperator"></div><span class="cometchat_chatboxmessagecontent">'+message+'</span>'+jqcc.mobilewebapp.getTimeDisplay(ts)+'</div><div style="clear:both;"></div></div>');
                    }
                }
            }
        });
    ...
    updateChatroomUsers: function(item, fetchedUsers){
        ...
        $.each(item, function(i, user){
            ...
            if(jqcc.cometchat.getcrAllVariables().users[user.id]!=1&&jqcc.cometchat.getChatroomVars('initializeRoom')==0&&jqcc.cometchat.getChatroomVars('hideEnterExit')==0){
                var ts = new Date();
                $("#currentroom_convotext").append('<div class="cometchat_chatboxalert" id="cometchat_groupmessage_0">'+user.n+'<?php echo $chatrooms_language[14]?>'+jqcc.mobilewebapp.getTimeDisplay(ts)+'</div><div style="clear:both;"></div>');
                setTimeout(function(){
                    jqcc.mobilewebapp.crscrollToBottom();
                }, 500);
            }
        ...
        for(user in jqcc.cometchat.getChatroomVars('users')){
            if(jqcc.cometchat.getChatroomVars('users').hasOwnProperty(user)){
                if(newUsers[user]!=1&&jqcc.cometchat.getChatroomVars('initializeRoom')==0&&jqcc.cometchat.getChatroomVars('hideEnterExit')==0){
                    var ts = new Date();
                    $("#currentroom_convotext").append('<div class="cometchat_chatboxalert" id="cometchat_groupmessage_0">'+jqcc.cometchat.getcrAllVariables().usersName[user]+'<?php echo $chatrooms_language[13]?>'+jqcc.mobilewebapp.getTimeDisplay(ts)+'</div><div style="clear:both;"></div>');
                    setTimeout(function(){
                        jqcc.mobilewebapp.crscrollToBottom();
                    }, 500);
                }
    ...
    ChatroomMessage: function(fromid, incomingmessage, incomingid, selfadded, sent, fromname){
        ...
        message = jqcc.cometchat.processcontrolmessage(controlparameters);
        if(message != ''){
            if($("#cometchat_groupmessage_"+incomingid).length>0){
                ...
                if(fromid!=jqcc.cometchat.getChatroomVars('myid')){
                    temp += ('<div><div class="cometchat_chatboxmessage you" id="cometchat_groupmessage_'+incomingid+'"><span class="cometchat_chatboxmessagefrom"><strong>');
                    if(typeof (jqcc.chatmobilewebapp)!=='undefined'){
                    ...
                    $("#currentroom_convotext").append('<div><div class="cometchat_chatboxmessage me" id="cometchat_groupmessage_'+incomingid+'"><span class="cometchat_chatboxmessagefrom"><strong>'+fromname+'</strong>'+separator+'</span><div class="userSeperator"></div><span class="cometchat_chatboxmessagecontent">'+message+'</span>'+sentdata+'</div><div style="clear:both;"></div></div>');
                 }
    ...
    mobilecreateChatroomSubmit: function(){
        ...
        jqcc('#createChatroomForm').find('#cometchat_chatroom_password').val('');
        jqcc.mobilewebapp.chatHeartbeatWebapp(1);
    },
    ...
    createChatroomSubmitStruct: function(){
        var name = $('#name').val();
        var type = $('#type').val();
        var password = $('#cometchat_chatroom_password').val();
        var room = {};
        if(name!=''&&name!=null){
