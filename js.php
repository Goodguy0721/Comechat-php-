...
if(!empty($_REQUEST['admin'])){
	if(file_exists(dirname(__FILE__).DIRECTORY_SEPARATOR.'writable'.DIRECTORY_SEPARATOR.$writable.DIRECTORY_SEPARATOR.'admin.js')&&DEV_MODE!=1){
		if(!empty($_SERVER['HTTP_IF_MODIFIED_SINCE'])&&strtotime($_SERVER['HTTP_IF_MODIFIED_SINCE'])==filemtime(dirname(__FILE__).DIRECTORY_SEPARATOR.'writable'.DIRECTORY_SEPARATOR.$writable.DIRECTORY_SEPARATOR.'admin.js')){
			header("HTTP/1.1 304 Not Modified");
			exit();
		}
		readfile(dirname(__FILE__).DIRECTORY_SEPARATOR.'writable'.DIRECTORY_SEPARATOR.$writable.DIRECTORY_SEPARATOR.'admin.js');
		$js = ob_get_clean();
	}else{
		...
		$fp = @fopen(dirname(__FILE__).DIRECTORY_SEPARATOR.'writable'.DIRECTORY_SEPARATOR.$writable.DIRECTORY_SEPARATOR.'admin.js','w');
		@fwrite($fp,$js);
		@fclose($fp);
	}
	$lastModified = filemtime(dirname(__FILE__).DIRECTORY_SEPARATOR.'writable'.DIRECTORY_SEPARATOR.$writable.DIRECTORY_SEPARATOR.'admin.js');
}else{
	if(!empty($cbfn)){
	...
	$cometchat = array();
	if(file_exists(dirname(__FILE__).DIRECTORY_SEPARATOR.'writable'.DIRECTORY_SEPARATOR.$writable.DIRECTORY_SEPARATOR.$theme.$cbfn.$color.$lang.$type.$name.'.js')&&DEV_MODE!=1){
		if(!empty($_SERVER['HTTP_IF_MODIFIED_SINCE'])&&strtotime($_SERVER['HTTP_IF_MODIFIED_SINCE'])==filemtime(dirname(__FILE__).DIRECTORY_SEPARATOR.'writable'.DIRECTORY_SEPARATOR.$writable.DIRECTORY_SEPARATOR.$theme.$cbfn.$color.$lang.$type.$name.'.js')){
			header("HTTP/1.1 304 Not Modified");
			exit();
		}

		readfile(dirname(__FILE__).DIRECTORY_SEPARATOR.'writable'.DIRECTORY_SEPARATOR.$writable.DIRECTORY_SEPARATOR.$theme.$cbfn.$color.$lang.$type.$name.'.js');
		$js = ob_get_clean();
	}else{
		...
		$fp = @fopen(dirname(__FILE__).DIRECTORY_SEPARATOR.'writable'.DIRECTORY_SEPARATOR.$writable.DIRECTORY_SEPARATOR.$theme.$cbfn.$color.$lang.$type.$name.'.js','w');
		@fwrite($fp,$js);
		@fclose($fp);
	}
	$lastModified = filemtime(dirname(__FILE__).DIRECTORY_SEPARATOR.'writable'.DIRECTORY_SEPARATOR.$writable.DIRECTORY_SEPARATOR.$theme.$cbfn.$color.$lang.$type.$name.'.js');
}
