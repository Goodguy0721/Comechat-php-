...
if(!empty($_REQUEST['admin'])){
	if(file_exists(dirname(__FILE__).DIRECTORY_SEPARATOR.'writable'.DIRECTORY_SEPARATOR.$writable.DIRECTORY_SEPARATOR.'admin.css')&&DEV_MODE!=1){
		if(!empty($_SERVER['HTTP_IF_MODIFIED_SINCE'])&&strtotime($_SERVER['HTTP_IF_MODIFIED_SINCE'])==filemtime(dirname(__FILE__).DIRECTORY_SEPARATOR.'writable'.DIRECTORY_SEPARATOR.$writable.DIRECTORY_SEPARATOR.'admin.css')){
			header("HTTP/1.1 304 Not Modified");
			exit();
		}
		readfile(dirname(__FILE__).DIRECTORY_SEPARATOR.'writable'.DIRECTORY_SEPARATOR.$writable.DIRECTORY_SEPARATOR.'admin.css');
		$css = ob_get_clean();
	}else{
		...
		$fp = @fopen(dirname(__FILE__).DIRECTORY_SEPARATOR.'writable'.DIRECTORY_SEPARATOR.$writable.DIRECTORY_SEPARATOR.'admin.css','w');
		@fwrite($fp,$css);
		@fclose($fp);
	}
	$lastModified = filemtime(dirname(__FILE__).DIRECTORY_SEPARATOR.'writable'.DIRECTORY_SEPARATOR.$writable.DIRECTORY_SEPARATOR.'admin.css');
}else{
	$type = 'core';
	$name = 'default';
	$subtype = '';
	...
	if(file_exists(dirname(__FILE__).DIRECTORY_SEPARATOR.'writable'.DIRECTORY_SEPARATOR.$writable.DIRECTORY_SEPARATOR.$theme.$type.$name.$cbfn.$color.'.css')&&DEV_MODE!=1){
		if(!empty($_SERVER['HTTP_IF_MODIFIED_SINCE'])&&strtotime($_SERVER['HTTP_IF_MODIFIED_SINCE'])==filemtime(dirname(__FILE__).DIRECTORY_SEPARATOR.'writable'.DIRECTORY_SEPARATOR.$writable.DIRECTORY_SEPARATOR.$theme.$type.$name.$cbfn.$color.'.css')){
			header("HTTP/1.1 304 Not Modified");
			exit();
		}
		readfile(dirname(__FILE__).DIRECTORY_SEPARATOR.'writable'.DIRECTORY_SEPARATOR.$writable.DIRECTORY_SEPARATOR.$theme.$type.$name.$cbfn.$color.'.css');
		$css = ob_get_clean();
	}else{
		...
		$css = minify(ob_get_clean());
		$fp = @fopen(dirname(__FILE__).DIRECTORY_SEPARATOR.'writable'.DIRECTORY_SEPARATOR.$writable.DIRECTORY_SEPARATOR.$theme.$type.$name.$cbfn.$color.'.css','w');
		@fwrite($fp,$css);
		@fclose($fp);
	}
	$lastModified = filemtime(dirname(__FILE__).DIRECTORY_SEPARATOR.'writable'.DIRECTORY_SEPARATOR.$writable.DIRECTORY_SEPARATOR.$theme.$type.$name.$cbfn.$color.'.css');
}
